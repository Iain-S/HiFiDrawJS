...
HiFiDrawGraphNetworkBinding.prototype.setNodesAndEdges = function (nodes, edges) {
    "use strict";
...
function HiFiDrawTableGraphBinding (hifidrawGraphNetworkBinding, tableDiv) {
    ...
    // Whenever the button is clicked, we want to be notified
    const button = this.table.find("input").first();
...
HiFiDrawTableGraphBinding.prototype.notifyGraph = function () {
    // Notify the graph-network binding that values may have changed
    "use strict";
    console.log("graph notified");
};
...
HiFiDrawTableGraphBinding.prototype.addRowToTable = function (src_val, conn_val, dest_val) {
    ...
    // Note, focus is lost if the user clicks a button
    ...
    let srcBox = makeSourceBox(src_val);
...
HiFiDrawTableGraphBinding.prototype.appendTo = function (parentElement) {
...
function makeTableWithAddButton () {
    "use strict";
    return $("<table>\n" +
        "       <thead>\n" +
        "         <tr>\n" +
...
function makeTable(drawingArea) {
    ...
    const newTable = makeTableWithAddButton();
...
function deleteRowFrom(tableObj, idx, drawingArea) {
    "use strict";
...
function deleteLastDataRowFrom(tableObj, drawingArea) {
    // This is a safe delete function, it will always leave the
    // headers and the add button.
    "use strict";
    ...
    if (tableBody.find("tr").length > 1) {
        deleteRowFrom(tableObj, tableBody.children("tr").length - 1, drawingArea);
    }
}
...
function addSampleData(tableObj, redrawFunc) {
    "use strict";
    addRow(tableObj, redrawFunc, "phone", "amp", "XLR<>XLR");
    addRow(tableObj, redrawFunc, "amp", "speakers");
    addRow(tableObj, redrawFunc);
}
...
// e.g. www.my-site.com?xx_key_1=a_thing&zz_key_2=why
//      will return a dict with "xx_key_1" and "zz_key_2" as keys
...
function addDataFromURL(serialisedData, tableObj, drawingDivID, redrawFunc) {
    ...
    unpackedData.edges.forEach(function (edge) {
        ...
        if (from_label && to_label) {
            addRow(tableObj, redrawFunc, from_label, to_label, edge.label);
        }
    });
}
